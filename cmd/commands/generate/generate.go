...
import (
	...
	"github.com/iufansh/iutool/cmd/commands"
	"github.com/iufansh/iutool/generate"
	"github.com/iufansh/iutool/generate/swaggergen"
)

var CmdGenerate = &commands.Command{
	UsageLine: "generate [command]",
	Short:     "Source code generator",
	Long: `▶ {{"To generate model func, controller, view based modelname:"|bold}}

     $ iutool generate mcv [modelname]

  ▶ {{"To scaffold out your entire application:"|bold}}

     $ iutool generate scaffold [scaffoldname] [-fields="title:string,body:text"] [-driver=mysql] [-conn="root:@tcp(127.0.0.1:3306)/test"]

  ▶ {{"To generate a Model based on fields:"|bold}}

     $ iutool generate model [modelname] [-fields="name:type"]

  ▶ {{"To generate a Model func based on modelname:"|bold}}

     $ iutool generate modelfunc [modelname]

  ▶ {{"To generate a controller:"|bold}}

     $ iutool generate controller [controllerfile]

  ▶ {{"To generate a CRUD view:"|bold}}

     $ iutool generate view [viewpath]

  ▶ {{"To generate a migration file for making database schema updates:"|bold}}

     $ iutool generate migration [migrationfile] [-fields="name:type"]

  ▶ {{"To generate swagger doc file:"|bold}}

     $ iutool generate docs

  ▶ {{"To generate a test case:"|bold}}

     $ iutool generate test [routerfile]

  ▶ {{"To generate appcode based on an existing database:"|bold}}

     $ iutool generate appcode [-tables=""] [-driver=mysql] [-conn="root:@tcp(127.0.0.1:3306)/test"] [-level=3]
`,
...
func mcv(args []string, currpath string) {
	if len(args) < 2 {
		beeLogger.Log.Fatal("Wrong number of arguments. Run: bee help generate")
	}
	sname := args[1]
	generate.GenerateMcv(sname, currpath)
}
...
func modelFunc(args []string, currpath string) {
	if len(args) == 2 {
		cname := args[1]
		generate.GenerateModelFunc(cname, currpath)
	} else {
		beeLogger.Log.Fatal("Wrong number of arguments. Run: bee help generate")
	}
}
